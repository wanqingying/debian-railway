...
- **OpenChamber serve** runs the web UI on `OPENCHAMBER_PORT` (default 3001, bound `0.0.0.0`, UI password from `OPENCHAMBER_UI_PASSWORD` falling back to `OPENCODE_SERVER_PASSWORD`) in **daemon launch mode**, and writes its own log to `$OPENCHAMBER_DATA_DIR/logs/openchamber-<port>.log` (the shell redirect in `entrypoint.sh` only captures the startup banner; `openchamber logs` tails the real one). It manages an embedded **opencode serve** on `OPENCODE_PORT` (default 4096, bound `OPENCHAMBER_OPENCODE_HOSTNAME` default `127.0.0.1`), which keeps basic auth from the standard opencode envs: user `opencode`, password `qingying` (override with `OPENCODE_SERVER_USERNAME`/`OPENCODE_SERVER_PASSWORD`). OpenChamber proxies the opencode API through its own server.
- **OpenCode 2 / OpenChamber 2 upgrade notes** (applied 2026-09-24):
  - **Version pair**: `@openchamber/web@2.0.1` + `@opencode/cli@2.0.15` (v2). The v1 package `opencode-ai` is no longer installed anywhere. OpenChamber 2.x requires OpenCode ≥2.0.15 (`server/lib/opencode/compatibility.js` → `MINIMUM_OPENCODE_VERSION`; 2.0.1 still declares 2.0.15) and will not fall back to v1. 2.0.1's own deps pin `@opencode/client`/`@opencode/schema` to 2.0.16, but those are client-side libs and do not reintroduce the 2.0.16 core regression.
  - **Updates: the in-app button is the live path, the pin is the durable one.** OpenChamber self-updates only a **daemon** instance: `POST /api/openchamber/update-install` spawns a detached `npm install -g @openchamber/web@latest`, exits (freeing the port), and the script restarts `openchamber serve` with the same flags. A **foreground** instance is assumed to be systemd-managed and gets a 409 (`Set OPENCHAMBER_SYSTEMD_UNIT ...`) — and Railway offers neither systemd nor the updater's container branch (no `/.dockerenv`, no `CONTAINER` env), which is exactly why the 2.0.0 foreground container could not update from the UI. `/usr` is ephemeral, so an in-place update lasts only until the next redeploy, which reinstalls `OPENCHAMBER_VERSION` from `scripts/install-tools.sh` — bump that pin to make an update permanent.
  - **`opencode.jsonc` must stay free of v2-unsupported keys**: OpenCode 2 still *accepts* the v1 config shape — `provider` with `npm`/`options`/`id`, plus `plugin`, `permission`, `autoupdate`, `watcher`, `mcp.<name>` — but it emits one `configuration normalization diagnostic` (`kind=unsupported`) per retired key on **every** config/provider/model resolution. Carrying the v1-only fields (`provider.commandcode.models.*.release_date`/`reasoning`/`attachment` × 24 models, top-level `server`, `compaction.prune`) made the idle server log ~750 lines/s (~35 MB / 3 min) and hold both opencode processes at ~40%/~16% CPU. The baked `opencode.jsonc` was cleaned of those keys (2026-09-24) and the provider still activates (verified). Re-add nothing unsupported.
  ...
  - **A `/skill` at position 0 works from OpenChamber 2.0.1 on** (it was broken in 2.0.0, upstream issue #3877). 2.0.0 routed any message whose first character is `/` through `POST /api/session/{id}/command`, but OpenCode 2's command endpoint resolves **only the command registry**, which is completely separate from the **skill registry** — so a leading skill name always 404'd with `Command not found: <skill>`. 2.0.1's chat router instead attaches a recognized skill to a normal prompt (`skills: [{id}]`, which injects the full skill body) — commit `61e6535`, PR #3895, plus `7f01569` "load skills live before routing a leading /skill". `/skill` mid-message (`请用 /req-to-ship 处理 …`) and the one-leading-space form (` /req-to-ship …`) still work. `opencode-config/opencode/commands/` stays intentionally absent — a command with the same name would shadow the native skill path, so no `/skill` wrapper commands.
...
- `opencode-config/opencode/skills/deep-research/` — portable, self-contained Agent Skill that turns a research question into a cited, verifiable report. `SKILL.md` (workflow + principles) plus `references/source-routing.md` (domain→authoritative-source registry with runnable API recipes), `subagent-briefs.md` (worker/verifier prompts and evidence schema), `report-template.md`, and `quality-checklist.md`. Requires no custom agent, plugin, or MCP server: it uses the built-in `websearch`/`webfetch`/shell tools and the host's built-in subagents (bounded to 2–3 workers per turn to avoid provider parallel-tool-call limits). Auto-triggers on 调研/研究/综述-style requests.
