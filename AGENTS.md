...
- **OpenCode 2 / OpenChamber 2 upgrade notes** (applied 2026-09-24):
  ...
  - **A `/skill` at position 0 does not work in OpenChamber 2.0.0** (upstream bug, do NOT "fix" it with wrapper commands). OpenChamber routes any message whose first character is `/` through `POST /api/session/{id}/command`, but OpenCode 2's command endpoint resolves **only the command registry**, which is completely separate from the **skill registry** — so a skill name always 404s with `Command not found: <skill>`. OpenCode itself is fine: `session.prompt` takes a `skills: [{id}]` param that injects the full skill body into the turn, and OpenChamber already uses it whenever `/skill` is **not at position 0**. So the working forms today are `/skill` mid-message (`请用 /req-to-ship 处理 …`) or with **one leading space** (` /req-to-ship …`) — both send exactly one `POST /api/session/{id}/prompt` with `skills:[{id}]` and no `/command` call. Refs: openchamber issue #3877, fixed by PR #3895 (commit `61e6535`) — **merged but not yet published** (npm `dist-tags` still `2.0.0`). `opencode-config/opencode/commands/` is therefore intentionally absent: once the fix ships, "a command with the same name still wins", so any same-named wrapper would permanently shadow the native skill path.
