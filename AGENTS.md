...
- **`scripts/install-tools.sh`** (baked to `/scripts/install-tools.sh`) — idempotent dev-toolchain bootstrap run at every start: apt packages (ffmpeg/lsof/unzip/jq/build-essential/fonts), `uv` + Python 3.12, pnpm 11.22.0 via corepack, Doppler CLI, Railway CLI, **OpenChamber** (`@openchamber/web`), **Playwright** (`@playwright/mcp` + `@playwright/cli`, CDP mode — no local browser), plus repair of a persisted `core/.venv` if present. Root-FS installs are wiped each redeploy, hence re-run on boot.
...
- `opencode-config/opencode/docs/playwright-setup.md` — Playwright usage guide (SSH reverse tunnel → local browser CDP).
