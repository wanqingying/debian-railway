...
- **`scripts/install-tools.sh`** (baked to `/scripts/install-tools.sh`) — idempotent dev-toolchain bootstrap run at every start: apt packages (ffmpeg/lsof/unzip/jq/build-essential/fonts), `uv` + Python 3.12, pnpm 11.22.0 via corepack, Doppler CLI, Railway CLI, **OpenChamber** (`@openchamber/web`), **agent-browser** (browser automation CLI, CDP mode — no local Chrome), **Playwright MCP** (`@playwright/mcp`, CDP mode), plus repair of a persisted `core/.venv` if present. Root-FS installs are wiped each redeploy, hence re-run on boot.
...
- `opencode-config/opencode/docs/agent-browser-setup.md` — agent-browser usage guide (SSH reverse tunnel → local browser CDP).
