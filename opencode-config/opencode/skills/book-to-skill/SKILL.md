...
## Container adaptation (opencode on Railway)

This copy lives in the opencode global skills directory on a persistent volume. Two environment facts override the generic defaults below:

- **This skill's own path** is `$XDG_CONFIG_HOME/opencode/skills/book-to-skill/` (`/workspace/.opencode/config/opencode/skills/book-to-skill/`). `scripts/extract.py` and `tools/` are there — use that path directly rather than hunting host roots in Step 2.
- **Where generated skills go**: write them to `$XDG_CONFIG_HOME/opencode/skills/<slug>/`, opencode's global skills root on the volume. Do **not** use `~/.agents/skills`: `$HOME` is `/root`, which is wiped on every redeploy, so anything written there is lost. opencode scans skills only at startup, so a newly generated skill appears after opencode restarts.
- Python extraction deps: install with `pip3 install …` or `uv pip install …`.
...
CANDIDATES=(
  "${XDG_CONFIG_HOME:-$HOME/.config}/opencode/skills/book-to-skill/scripts/extract.py"
